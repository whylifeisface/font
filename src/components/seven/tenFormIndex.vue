<template>
    <el-form :model="formModel" ref="formRef" :rules="rule">
        ...
        <div v-else-if="status==1">
            <el-form-item >
                <el-select v-model="formModel." @visible-change="vis1">
                    <el-option v-for="(o,index) in options0" :key="index " :label="o" :value="o"></el-option>
                </el-select>
        ...
        <div v-else-if="status==2">
            <el-form-item>
                <el-input v-model="formModel.PmsProduct.name" placeholder="商品名称/商品货号" @input="inputIn"></el-input>
...
import { TObject } from '@/components/utils/TObject';
...
console.log(route.query.Myquery);
       ...
       let M:O = JSON.parse( decodeURIComponent( route.query.allData + '' ))
        ...
        formModel = TObject<O>(M)
    //    formModel = twoObject(M,formModel);
}
// let options0 = ref([])
